...
export const ConexoesUI = () => {//receber o token de conexões
  //todos usestates dessa pagina
  const [connectionName, setConnectionName] = useState('');//conexão a ser criada
  ...
  //ajudar na criação da tabela de conexões por favor ignorar os nomes contraintuitivos
  function createData(name, calories, fat, carbs, protein) {
    return { name, calories, fat, carbs, protein };
  }
  //requisições e useffect
  const request_qrCode = async ()=>{
    setQrLoading(true)
    console.log('teste')
    const token = localStorage.getItem('token');
    if(connectionName.includes('_')){
      alert('O caractere "_" não é permitido para gerar uma nova conexão');
      setQrLoading(false);
      return;
    }
    try{
      console.log('INICIANDO REQUEST QR CODE')
      const response = await axios.post(`${BACKEND_URL}/qrcode`,{
  ...
  const DeleteConnection = async()=>{
    ...
    try{
      ...
      const response = await axios.post(`${BACKEND_URL}/delete_connection`,{
  ...
  useEffect(()=>{
    ...
    if(qrCode){
      try{
        const token = localStorage.getItem('token');
        const response_is_connected = await axios.get(`${BACKEND_URL}/is_wpp_connected`, {
